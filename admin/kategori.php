			  ...
			  <div class="page-content">
				<div class="dwqa-container" >
				  <form method="post" class="dwqa-content-ask-form" enctype="multipart/form-data">
					...
					<div class="col-md-5 col-xs-6">
					<button class="btn btn-success btn-sm" type="submit" name="save" value="Can change">Save
					</button>
					</div>
					</div>
					</p>
				...
				<?php
					...
					$sql_data = "insert into kategori (kategori, status) values
					('$name','$status')";
						if(mysqli_query($conn,$sql_data))
					{
						echo "<script>alert('Data di simpan');document.location='kategori.php'</script>";
					} else{
						echo "<script>alert('Error');document.location='kategori.php'</script>";
					}
						mysqli_close($conn);
				}
				?>
